...
class GoogleMapsScraper:
    ...
    def sort_by(self, url, ind):
        ...
        recent_rating_bt = self.driver.find_elements(
            By.XPATH, "//div[@role='menuitemradio']"
        )[ind]
    ...
    def get_places(self, keyword_list=None, save=False):
        ...
        for i, search_point_url in enumerate(search_point_url_list):
            ...
            try:
                ...
                if "place" in new_url:
                    ...
                    df_places = pd.concat([df_places, place_info_df], ignore_index=True)
                    continue
    ...
    def __parse_place(self, response, url):
        ...
        try:
            place["name"] = response.find(
                "h1", class_="DUwDvf fontHeadlineLarge"
            ).text.strip()
        except Exception as e:
            place["name"] = None
        ...
        try:
            place["overall_rating"] = float(
                response.find("div", class_="F7nice ")
                .find("span", class_="ceNzKf")["aria-label"]
                .split(" ")[1]
            )
        except Exception as e:
        ...
        try:
            place["n_photos"] = int(
                response.find("div", class_="YkuOqf")
                .text.replace(".", "")
                .replace(",", "")
                .split(" ")[0]
            )
        ...
        try:
            place["opening_hours"] = response.find("div", class_="t39EBf GUrTXd")[
                "aria-label"
            ].replace("\u202f", " ")
        except:
    ...
    def _gen_search_points_from_square(self, keyword_list=None):
        ...
        return [
            f"https://www.google.com/maps/search/{keyword_list[0]}/@{str(101.9758)},{str(4.2105)},{str(15)}z"
        ]
