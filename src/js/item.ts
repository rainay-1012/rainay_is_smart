...
export async function initItemPage() {
  ...
  const onItemFormSubmit = async (evt: SubmitEvent) => {
    ...
    await withBlock("#item-modal .modal-content", {
      ...
      try {
        ...
        showMessage(`Item successfully ${data.id ? "modified" : "added"}.`, {
  ...
  const procurementItemTemplate = document.querySelector("#item-row-template");
        ...
        buttons: [
          TableAction.generateExportButtonConfig({ columns: [1, 3, 4, 5] }),
          {
            ...
            action: async function () {
              const data = itemTable.rows({ selected: true }).data().toArray();
            ...
            action: function (e, dt, node, config) {
              ...
              data.forEach((row) => {
                const itemElement: HTMLElement = (
                  procurementItemTemplate.content.cloneNode(true) as HTMLElement
                ...
                mapToElement([
                  ...
                  { selector: `#${id} input[name='name']`, value: row.name },
                ]);
              });
    ...
    initComplete: (settings, json) => {
      ...
      TableAction.attachListeners({
        ...
        callback(this, event, d) {
          mapToElement([
            { selector: "input[name='id']", value: d.id },
            { selector: "input[name='name']", value: d.name },
          ]);
  ...
  const categorySuggestions = initialData.categories.map((category: any) => ({
    type: "table",
    tableId: "item-table",
